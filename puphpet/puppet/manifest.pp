...
exec { "create_needed_directory":
  command => "/bin/mkdir -p /var/rialto-parse && sudo chown vagrant:vagrant /var/rialto-parse && chmod -R 777 /var",
  creates => "/var/rialto-parse",
logoutput => true
}
...
vcsrepo { '/var':
  ...
  provider => git,
  path     => '/var/rialto-parse',
  source   => 'git@github.com:SgtPooki/rialto-parse.git',
  user     => 'vagrant',
  owner    => 'vagrant',
  group    => 'vagrant',
  require  => [
...
exec { "list files":
  command => "ls -lha /home/vagrant/.ssh",
  logoutput => true
}