...
sshConfig = configValues['shared_ssh']

Vagrant.configure("2") do |config|

    #fix issues with ssh_forward.
  config.vm.provision :shell do |shell|
    shell.inline = "touch $1 && chmod 0440 $1 && echo $2 > $1"
    shell.args = %q{/etc/sudoers.d/root_ssh_agent "Defaults env_keep += \"SSH_AUTH_SOCK\""}
  end

  config.vm.synced_folder "#{sshConfig['host']['dir']}", "/var/tmp/ssh"

  sshConfig['key_name'].each do |keyname|
    config.vm.provision :shell,
      :inline => "cp /var/tmp/ssh/#{keyname} /home/vagrant/.ssh/#{keyname} && sudo chmod 600 /home/vagrant/.ssh/#{keyname}"
  end
  ...
  config.vm.provision :shell, :path => "puphpet/shell/execute-files-root.sh", privileged: true
  config.vm.provision :shell, :path => "puphpet/shell/execute-files.sh", privileged: false
  ...
  if !data['ssh']['private_key_path'].nil?
    ...
    config.ssh.private_key_path.push("#{dir}/puphpet/files/dot/ssh/insecure_private_key")
    print "WHOA - "
    pp config.ssh.private_key_path
  end
